...
> `[iOS Networking/WtdConnection.swift: pingTimer]` — name files in repositories you were not
...
|---|---|---|---|---|
| `GET /token` | `{"token": ""}` without `-c` [GT] | Identical **body**, and *always* empty — no auth exists. Headers differ: wtd sends `Cache-Control: no-store` (#116) and a charset on `Content-Type`; ttyd sends neither [LAB: probed 2026-09-02, ttyd 1.7.4] | Match (body byte-exact) | **The iOS app no longer fetches this** — the token round trip was deleted when it became wtd-only. What keeps the endpoint is ttyd wire parity (`server_test.go` pins the body byte-exact and the conformance suite diffs it against real ttyd 1.7.4), plus one live consumer in `cmd/wtd/web/terminal.html`, plus third-party clients that may be less forgiving of a 404. |
...
| `GET /ws` upgrade, **no** subprotocol offered | 101, no echo, connection held open [LAB: probed 2026-07-28, ttyd 1.7.4] | Identical | Match | wtd closed these 1008 until #36, which was a real incompatibility: a hand-rolled client that worked against ttyd did not work against wtd. |
| `GET /ws` upgrade, other subprotocols **without** `tty` | Drops the TCP connection with no HTTP response at all [LAB: probed 2026-07-28, ttyd 1.7.4] | **HTTP 400** before the upgrade | **Diverges, deliberately** | ttyd's silent reset is indistinguishable from a network failure. A 400 carries the same refusal in a form a client can read and log. Safe to diverge: a client offering only other subprotocols is not a ttyd client, so nothing depends on the reset. |
...
| Ping/pong | libwebsockets answers pings [GT — the iOS 20 s ping has run against production since v1, iOS Networking/WtdConnection.swift: pingTimer]; server-side ping/idle defaults **UNVERIFIED** | Answers pings; server pings every 30 s and closes after **three** go unanswered (90 s); idle floor 60 s | Match + specified | The 20 s client ping must always land inside any timeout, and browsers cannot ping at all. wtd gave up on the *first* missed pong until #40, which made the real deadline 50 s and broke its own 60 s floor. What is consulted is the pong, not traffic in general. `ws-protocol.md` section 10. |
| Flow control (`'2'`/`'3'`) | Opcodes exist [GT]; server semantics **UNVERIFIED** | Accepted as no-ops; backpressure via bounded buffers + PTY read-stop (argless only — a named client past the backlog budget is dropped instead, `ws-protocol.md` section 9) | Match (for known clients) | No known client sends them [iOS Networking/WtdProtocol.swift: case pause]. Future use gated behind a `flow-control` feature flag. |
| Close codes | **UNVERIFIED** | Defined table (`ws-protocol.md` section 13) | Specified | The iOS client **does** branch on them now: `shouldRetry` treats 1000, 1002, 1008 and 1009 as final and retries the rest with backoff [iOS Networking/WtdConnection.swift: shouldRetry]. So the table is load-bearing rather than free — changing a code changes whether a shipped phone reconnects. |
...
| Resize application | Applied per frame; coalescing behavior **UNVERIFIED** but the shipped redraw kick works against it [GT] | One `TIOCSWINSZ` per frame, in order, never coalesced — hard requirement | Match | The blank-screen kick dies silently under server-side debouncing. wtd now performs that kick itself, once per session (`hub.kick`). The iOS client still has its own, but it is **gated on the server advertising `scrollback-replay`** [iOS Networking/WtdConnection.swift: scheduleRedrawKick], so against this server it does not fire and against an un-upgraded one it still does. Both paths must keep working: the gate is the only thing that tells them apart. `ws-protocol.md` section 7. |
| Window title frame | The process title | Argless: `bash (<hostname>)`, or `<start-command> (<hostname>)` when one is configured. Named: `<arg> (<hostname>)` | Divergence, wire-visible | The iOS client renders this verbatim in its nav bar [iOS Views/TerminalScreenView.swift, `.navigationTitle(connection.windowTitle ?? profile.name)`], so a deep-link profile's title reads as the session name rather than a binary path. Nothing parses it (`ws-protocol.md` section 6). The argless value changed again when the picker was retired; no shipped client reaches that path (its session name is non-optional [iOS Networking/WtdConnection.swift: sessionName]). |
...
`wtd -listen <resolved-ip>:<port>` [bin/wt-serve], on the same port. There is no start command
any more: wtd runs `dtach -A` itself for a named connection and a plain shell for an argless one.
...
| `-p <port>` | Listen port | `WT_PORT`, default 7681 | Match | 7681 is also the iOS profile default [iOS Models/ServerProfile.swift: port], which is why retiring ttyd moved `wtd` *onto* this port rather than keeping its own: every existing profile reaches the new server without being edited. |
...
|---|---|---|---|
| `-c user:pass` | HTTP basic auth | **Omitted — and `wtd` MUST refuse to start if `WT_AUTH` is non-empty** (fail closed, not silently unauthenticated). Since ttyd was retired there is no second server to fall back to, so `install.sh` MUST make the same refusal *before it writes anything* — the operator finds out while the previous server is still running, not from the journal of a box that no longer serves. | The nuance matters (`CLAUDE.md`): basic auth breaks Safari and every iOS *browser* (WebKit omits credentials on the WS upgrade) but **not** the native app, which is not a browser and can set `Authorization` itself. Note the estimate has grown: that client once carried a `BasicCredentials` seam plumbing the header onto both the `/token` GET and the upgrade, and it was **deleted** when the client became wtd-only — a server that refuses to start with `WT_AUTH` set made the seam unreachable, so it was carrying no weight. So auth is a real *future* option, not a dead end — but the client half is now a re-add rather than a UI on existing plumbing, making it a **two-repo change** (credential path *and* UI there, verification here), gated behind the reserved `auth-basic` feature flag and tracked in #27. Until then, a configured-but-ignored `WT_AUTH` would mean an operator believes auth is on when it is not; refusing is the only honest response, and it costs one config edit. |
...
| `-b <path>` | Base path (serve under a URL prefix) | Omitted in v1; `base-path` feature name reserved | **This one matters and is not dismissed lightly**: the iOS client already supports `pathPrefix` for reverse-proxy deployments [iOS Models/ServerProfile.swift: pathPrefix] — a shape `CLAUDE.md` notes the client handles but this repo never documented. The v1 contract: the proxy **must strip the prefix**, so `GET /prefix/ws` arrives at wtd as `/ws`; the client's `pathPrefix` and a stripping proxy compose correctly today. What is lost without `-b`: deployment behind a *non-stripping* proxy. If that shape ever becomes real, implement `base-path`, advertise it in `features[]`, and serve under both `/` and the prefix. wtd's own pages use relative URLs specifically so proxied deployments work. |
| `-6` | IPv6 listener | Omitted as a flag | `WT_BIND` accepts a literal IPv6 address (refusing `::`), which covers the realistic case (a tailnet IPv6 literal). The `tailscale` keyword stays IPv4 (`tailscale ip -4` [bin/wt-bind.sh]); a `tailscale6` keyword is possible future work, not speculative abstraction today. |
| `-t key=value` | Client UI options (theme, font...) delivered via the preferences frame | Omitted; wtd sends `'2'` + `{}` | The iOS client ignores preferences entirely [iOS Networking/WtdProtocol.swift: case preferences]; wtd's own pages style themselves. An option channel with zero consumers is contract surface for free. |
...
| `WT_PICKER` | Start command path | **Retired — warn and ignore** | wtd attaches to dtach itself, so there is nothing for the key to point at [bin/wt-serve]. Warn rather than refuse, following `WT_WEB_PORT`: a stale key must not keep a box off the network, and a picker path is not an access control. Honouring it as an alias for `-start-command` would mean no existing box ever migrated, since `install.sh` never clobbers a config. An operator who genuinely wants an external start command passes `-start-command` to wtd. |
| `WT_PROJECTS` | Exported to `wt` when set | Honored, passed as `-projects-file`, and read by wtd itself for `GET /api/v1/projects`, `project:` creates and deep-link working directories | Passed as a flag rather than exported, because the environment hop is a silent-failure class: sourcing a config creates shell variables, so a key a *child* reads from the environment never arrives, with no error anywhere. `WT_PROJECTS` was broken exactly that way once. Still exported as well, for `wt` over SSH, which takes no flags. |
...
| `WT_DIR` | *(was not a config key: every consumer read the environment, so setting it in the config reached nothing — #28)* | **Config key, passed as `-session-dir`.** Default: the service user's `$HOME/.dtach` [cmd/wtd/config.go: func (s *server) sessionDir]. Also exported, for `wt` over SSH. | A flag rather than an export is what makes #28 unrepresentable: a value that does not arrive is a value the launcher did not pass, visible in the process's own command line, instead of a documented key that is silently inert. |
...
terminals=dtach)` [bin/wt-serve], the resolved bind, which is the line
