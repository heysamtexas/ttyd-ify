...
The xterm files are MIT licensed; see `LICENSE.xterm`, `LICENSE.addon-fit`,
`LICENSE.addon-webgl` and `LICENSE.addon-web-links`. Intel One Mono is under the SIL Open Font
License 1.1 (`LICENSE.intel-one-mono`), which reserves the name "Intel" for modified versions.
So the files here are byte-identical to the release, not subset or converted.
...
| `IntelOneMono-Medium.woff2`, `IntelOneMono-Bold.woff2` | [Intel One Mono](https://github.com/intel/intel-one-mono) `woff2.zip` | V1.4.0 |
...
b0bd66dcc855ceaab935ffb985fa45482a466351b9f36bee84f6b4ffff7c570a  intel-one-mono-V1.4.0-woff2.zip
```
...
(cd cmd/wtd/web/vendor && sha256sum xterm.js xterm.css addon-fit.js addon-webgl.js addon-web-links.js \
  IntelOneMono-Medium.woff2 IntelOneMono-Bold.woff2 > SHA256SUMS)
```
...
Astigmatism mode (#140) uses two weights, Medium for text and Bold for bold, and they are the
only faces here. The terminal page never asks for Regular, Light or the italics, and the
browser slants the upright face for italic.

```sh
curl -fsSL -o /tmp/iom.zip https://github.com/intel/intel-one-mono/releases/download/V1.4.0/woff2.zip
sha256sum /tmp/iom.zip        # compare with the line above
unzip -j /tmp/iom.zip 'woff2/IntelOneMono-Medium.woff2' 'woff2/IntelOneMono-Bold.woff2' -d cmd/wtd/web/vendor
curl -fsSL -o cmd/wtd/web/vendor/LICENSE.intel-one-mono \
  https://raw.githubusercontent.com/intel/intel-one-mono/V1.4.0/OFL.txt
```
