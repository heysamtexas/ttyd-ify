...
Everything here is MIT licensed; see `LICENSE.xterm`, `LICENSE.addon-fit` and
`LICENSE.addon-webgl`.
...
`addon-webgl` has no `peerDependencies` to pin it, so match it to `xterm` by release rather
than by range: 0.19.0 was published 51 seconds after 6.0.0. The addon that *looks* like the
safer, more portable choice — `@xterm/addon-canvas` — is not one. It is still at 0.7.0 from
April 2024, peered to `@xterm/xterm@^5`, and was never updated for 6. So the renderer ladder
here has two rungs, WebGL and DOM, and no middle.
...
sha256sum /tmp/xterm.tgz /tmp/fit.tgz /tmp/webgl.tgz   # record these in this file

# All three unpack to /tmp/package, so extract and copy one at a time or they overwrite:
#   xterm.tgz -> lib/xterm.js, css/xterm.css, LICENSE
#   fit.tgz   -> lib/addon-fit.js, LICENSE
#   webgl.tgz -> lib/addon-webgl.js, LICENSE
tar xzf /tmp/xterm.tgz -C /tmp   # ...copy, then repeat for fit and webgl. Then:
(cd cmd/wtd/web/vendor && sha256sum xterm.js xterm.css addon-fit.js addon-webgl.js > SHA256SUMS)
```
