...
| `WT_PICKER` | Start command path | Honored | Default `/usr/local/bin/wt` [bin/wt-serve]. **Not reported in `/api/v1/meta`** — `terminalPath` there is the WebSocket path (`/ws`), never the start command. The spec was corrected in `fcb66f6`; this row was the copy it missed (#19). |
